...
.filter-bar {
  ...
  background: rgba(15, 23, 42, 0.40); backdrop-filter: blur(12px);
  border: 1px solid rgba(148, 163, 184, 0.10); border-radius: 12px;
}
.filter-bar input {
  flex: 1; min-width: 200px; padding: 8px 14px;
  background: rgba(30, 41, 59, 0.50); border: 1px solid rgba(148, 163, 184, 0.15);
  border-radius: 8px; color: #e2e8f0; font-size: 13px; outline: none;
  transition: border-color 0.2s;
}
.filter-bar input:focus { border-color: rgba(59, 130, 246, 0.5); }
.filter-bar input::placeholder { color: #64748b; }
.filter-toggle {
  display: flex; align-items: center; gap: 6px;
  padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 500;
  cursor: pointer; transition: all 0.2s; white-space: nowrap;
  background: rgba(30, 41, 59, 0.40); border: 1px solid rgba(148, 163, 184, 0.12);
  color: #94a3b8;
}
.filter-toggle:hover { border-color: rgba(148, 163, 184, 0.3); color: #cbd5e1; }
.filter-toggle.active { color: #e2e8f0; font-weight: 600; }
.filter-toggle .dot {
...
function renderFilterBar(prefix) {
  return '<div class="filter-bar">' +
    ...
    '<span class="filter-toggle active" data-metric="schedule" onclick="toggleMetric(\'' + prefix + '\',\'schedule\',this)" style="border-color:rgba(34,197,94,0.3)"><span class="dot" style="background:#22c55e"></span>排班率</span>' +
    '<span class="filter-toggle" data-metric="correct" onclick="toggleMetric(\'' + prefix + '\',\'correct\',this)" style="border-color:rgba(59,130,246,0.3)"><span class="dot" style="background:#3b82f6"></span>正确率</span>' +
    '<span class="filter-toggle" data-metric="hub" onclick="toggleMetric(\'' + prefix + '\',\'hub\',this)" style="border-color:rgba(139,92,246,0.3)"><span class="dot" style="background:#8b5cf6"></span>HUB</span>' +
    '<span class="filter-toggle" data-metric="punch" onclick="toggleMetric(\'' + prefix + '\',\'punch\',this)" style="border-color:rgba(249,115,22,0.3)"><span class="dot" style="background:#f97316"></span>打卡率</span>' +
...
function renderOverview() {
  ...
  '<div class="cards-grid">' +
    ...
    metricCard('\u23F0 日超8H合计', o['日超8H'].total_hours + 'h', o['日超8H'].rate,
  ...
  var tableRows = DATA.departments.map(function(dept, i) {
    ...
    return '<tr>' +
      ...
      '<td>' + drillData(o8D.total_hours + 'h', '日超8H', o8D.employees) + '</td>' +
...
function renderDepartment(dept) {
  ...
  '<div class="cards-grid">' +
    ...
    metricCard('\u23F0 日超8H合计', s['日超8H'].total_hours + 'h', s['日超8H'].rate,
  ...
  subRows += '<tr class="summary"><td>&#x1F4CA; ' + dept.name + ' 汇总</td>' +
    '<td>' + s.total + '</td>' +
    '<td>' + s['日超8H'].total_hours + 'h (' + s['日超8H'].rate + ')</td>' +
    '<td>' + s['排班']['排班率'] + '</td>' +
    '<td>' + s['排班正确']['正确率'] + '</td>' +
    '<td>' + s['排班']['未排班'] + '</td>' +
    '<td>' + punchD['打卡率'] + '</td>' +
